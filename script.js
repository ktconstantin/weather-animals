const API_KEY = 'cd15ea96100fa0cba5b655beee1bf0b3';
...
//   const response = await fetch(api.openweathermap.org/data/2.5/forecast?lat={47.626785}&lon={-117.406181}&appid={cd15ea96100fa0cba5b655beee1bf0b3});
...
// populate dropdown with all 50 states
...
// create label for state dropdown
...
stateContainer.prepend(stateLabel);

countryList.onchange = () => {
  selectedCountry = countryList.value;

  // show a dropdown for US States if selecting USA as country
  if (selectedCountry === 'United States of America (the)') {
...
// assign value of selected state
let selectedState;
stateList.onchange = () => {
  selectedState = stateList.value;
};
...
submitButton.addEventListener('click', () => {
  let city = cityInput.value[0].toUpperCase() + cityInput.value.slice(1).toLowerCase();
  userInput.city = city;
  ...
  // set values in object for fetch API
  userInput.state = selectedState;
  countryIndex = COUNTRY_DATA.findIndex(country => country.name === selectedCountry);
  userInput.country = COUNTRY_DATA[countryIndex].code;

  // set display in header
  currentCity.textContent = city;
  currentCountry.textContent = userInput.country;
...
async function getCoordinates() {
  let q;
  if (userInput.state === undefined) {
    q = `${userInput.city},${userInput.country}`;
  } else {
    q = `${userInput.city},${userInput.state},${userInput.country}`;
  }

  const geocodingResponse = await fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${q}&limit=5&appid=${API_KEY}`, {mode: 'cors'});
  const geocodingData = await geocodingResponse.json();

  let lat = geocodingData[0].lat;
  let lon = geocodingData[0].lon;
  
  const weatherResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}`, {mode: 'cors'});
  const weatherData = await weatherResponse.json();

  currentWindSpeed = (weatherData.wind.speed * METERS_PER_SEC_TO_MPH).toFixed();
  windSpeed.textContent = currentWindSpeed + ' mph';
}

// speed in mph
const ANIMAL_SPEEDS = [
  {name: 'Emperor penguin', speed: 5, mode: 'swimming', img: 'penguin.jpg'},
  {name: 'Pig', speed: 10, mode: 'running', img: 'pig.jpg'},
  {name: 'Bulldog', speed: 15, mode: 'running', img: 'bulldog.jpg'},
  {name: 'Squirrel', speed: 20, mode: 'running', img: 'squirrel.jpg'},
  {name: 'Turkey', speed: 25, mode: 'running', img: 'turkey.jpg'},
  {name: 'Housecat', speed: 30, mode: 'running', img: 'cat.jpg'},
  {name: 'Grizzly bear', speed: 35, mode: 'running', img: 'bear.jpg'},
  {name: 'Deer', speed: 40, mode: 'running', img: 'deer.jpg'},
  {name: 'Greyhound', speed: 45, mode: 'running', img: 'greyhound.jpg'},
  {name: 'Lion', speed: 50, mode: 'running', img: 'lion.jpg'},
  {name: 'Quarter horse', speed: 55, mode: 'galloping', img: 'horse.jpg'},
  {name: 'Gazelle', speed: 60, mode: 'running', img: 'gazelle.jpg'},
  {name: 'Common Swift', speed: 67, mode: 'flying level', img: 'swift.jpg'},
  {name: 'Cheetah', speed: 75, mode: 'running', img: 'cheetah.jpg'},
  {name: 'Golden Eagle', speed: 80, mode: 'flying level', img: 'eagle-slow.jpg'},
  {name: 'Gyrfalcon', speed: 90, mode: 'flying level', img: 'gyrfalcon-slow.jpg'},
  {name: 'Saker falcon', speed: 95, mode: 'flying level', img: 'saker.jpg'},
  {name: 'White-throated needletail', speed: 105, mode: 'flying level', img: 'needletail.jpg'},
  {name: 'Gyrfalcon', speed: 123, mode: 'diving', img: 'gyrfalcon-fast.jpg'},
  {name: 'Golden eagle', speed: 150, mode: 'diving', img: 'eagle-fast.jpg'},
  {name: 'Peregrine falcon', speed: 200, mode: 'diving', img: 'peregrine.jpg'},
];

const METERS_PER_SEC_TO_MPH = 2.236936;

const getClosestAnimal = (windSpeed) => {
  let closest = ANIMAL_SPEEDS.reduce((a, b) => {
    return Math.abs(b.speed - windSpeed) < Math.abs(a.speed - windSpeed) ? b : a;
  });

  return closest;
};

const convertButton = document.getElementById('convert-wind-speed');
const windAnimal = document.getElementById('wind-animal');
const currentAnimalImageBox = document.getElementById('current-animal-img');

convertButton.addEventListener('click', () => {
  let animal = getClosestAnimal(currentWindSpeed);
  windAnimal.textContent = `${animal.name} while ${animal.mode}`;

  const animalImage = document.createElement('img');
  animalImage.src = `img/${animal.img}`;
  console.log(currentAnimalImageBox);
  currentAnimalImageBox.appendChild(animalImage);
});